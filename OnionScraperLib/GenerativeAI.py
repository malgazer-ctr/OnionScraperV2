        ...
        if len(data_dict.get('victimName', '')) > 0 and len(data_dict.get('victimBizType', '')) > 0:
            if '不明' not in data_dict['victimName'] and '不明' not in data_dict['victimBizType']:
                ...
                promptText = (
                    ...
                    f'※いずれにも該当しない場合は{victimsNamebyAI}をもとに調査してください'
                    '日本国内に子会社・関連会社・支店・研究所などの拠点を持っている。もしくは日本企業（公開会社・非公開会社問わず）や日本政府機関、日本に本社を置く国際企業と資本提携・合弁・包括的な事業提携を結んでいる。'
                    ...
                    '調査時には、企業名の和英・略称・旧社名など表記揺れを考慮し、商業登記、企業の公式発表、信頼性の高いニュース記事や業界レポートなど複数の独立した情報源を優先してください。'
                    ...
                    '- 日本企業と関係があると判断できる場合：文頭に「あり。」と書き、それ以外の場合は特に「なし」や「不明」の記載の必要なし。'
                    '- 具体的な関係性を続けて200文字以内で説明する'
                    # '- 日本企業や主要産業、日本政府と関連する企業かどうかが重要な情報のため、各項目について詳細な調査報告は不要です。'
